                                                       ...
                                                       std::string Nspace = "")
{
    ...
    for (size_t partNum = (nameOfConstruction == "$this"); partNum < parts.size(); partNum++) {
...
void PhpChepinOperatorParser::ParseCode()
{
    ...
    if (TryFindOperatorWithStructure("while({\f", findInfo)) {
        ...
        std::vector<std::string> withoutNameOfFunction(findInfo.size() - 1);

        for (size_t it = 1; it < findInfo.size(); it++) {
            withoutNameOfFunction[it - 1] = findInfo[it];
        }

        ParsePartsOfConstruction(withoutNameOfFunction, findInfo[0]);
        return;
    }
    ...
    for (const auto &variable : valuablesData_) {
        ...
        for (const auto &usage : variable.second) {
            for (const auto &instance : usage) {
                ...
                if (instance == "0=" || instance == "0++" || instance == "1++" || instance == "0--"
                    || instance == "1--") {
                    cntWhenModifying++;
                }
            }
